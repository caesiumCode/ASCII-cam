...
Program::Program(uint w)
{
    ...
    m_ascii_drawable.setCharacterSize(getOptimalCharacterSize());
...
void Program::run()
{
    while (m_window.isOpen())
    {
        ...
        while (m_window.pollEvent(event))
        {
            ...
            else if (event.type == sf::Event::KeyReleased && event.key.code == sf::Keyboard::Up) m_ascii_cam.increaseThreshold();
            else if (event.type == sf::Event::KeyReleased && event.key.code == sf::Keyboard::Down) m_ascii_cam.decreaseThreshold();
        }
...
uint Program::getOptimalCharacterSize()
{
    uint character_size = 1;
